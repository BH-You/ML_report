...
Instead, we just used simple approach that uses only one citation count value at fixed date. There exists a date difference between the citation count retrieval date. Considering the highly-cited paper are likely to highly-cited further, there is a strong relationship between citation count at the time of writing and fixed point. This simple approximates still able to approximate the citation count effectively while saving the data size a lot.\\
